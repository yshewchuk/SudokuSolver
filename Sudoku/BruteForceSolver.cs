...
namespace Sudoku
{
    public class BruteForceSolver : Solver
    {
        ...
        public SudokuBoard Solve(SudokuBoard initialBoard)
        {
            ...
            SudokuBoard solution = Solve(initialBoard, new Coordinate2D(0, 0), 0.0, 1.0 / SudokuBoard.Dimension);
        ...
        private SudokuBoard Solve(SudokuBoard board, Coordinate2D position, double baseProgess, double progressIncrement)
        {
            ...
            foreach (SudokuBoard.Symbol symbol in Symbols)
            {
                board[position] = symbol;
                if (RowIsValid(position, board)
                    && ColumnIsValid(position, board)
                    && BoxIsValid(position, board))
                {
                    if (terminal)
                    {
                        return board;
